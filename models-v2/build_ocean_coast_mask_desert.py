"""Masque de cote du desert (carte d'ocean *ocean-map-desert*) pour le rendu moderne de la mer et des oasis.
...
SOURCE = ROOT / 'data/goal_src/jak3/levels/desert/desert-ocean.gc'
TARGET = ROOT / 'engine-src/game/graphics/opengl_renderer/ocean/OceanCoastMaskDesert.h'
ORIGIN = (-1048576.0 / 4096, -1048576.0 / 4096)   # start-corner de *ocean-map-desert*
LEVEL = 36864.0 / 4096   # y du start-corner : 9 m
...
def main():
    ...
    trans = []
    for item in forms(section(text, '*ocean-trans-indices-desert*'), 'ocean-trans-index'):
    ...
    near = [array(i, 'uint16', 16) for i in forms(section(text, '*ocean-near-indices-desert*'), 'ocean-near-index')]
    masks = [array(i, 'uint8', 8) for i in forms(section(text, '*ocean-mid-masks-desert*'), 'ocean-mid-mask')]
    assert len(trans) == 2304, len(trans)
    print('Desert : transitions', len(trans), 'near', len(near), 'masques', len(masks))
    ...
    observations = [{'name': name, 'xz': [x, z], 'wet': wet(x, z)} for name, x, z in (
        ('oasis de la zone E (eau)', 590.0, 2600.0),
        ('sable devant la porte de Spargus (sec)', 2266.4, 258.1),
        ('mer a l ouest de la zone D', 3890.0, 720.0))]
...
// Genere par models-v2/build_ocean_coast_mask_desert.py depuis *ocean-map-desert* (desert-ocean.gc).
// Meme decodage que OceanCoastMask (Spargus) : 96 m -> 24 m -> 3 m ; un bit de masque leve = sec.
class OceanCoastMaskDesert {{
 public:
...
'''
    ...
    (ROOT / 'models-v2/ocean-coast-mask-desert-report.json').write_text(json.dumps(report, indent=2))
    print(json.dumps(report['observations'], indent=1), 'cellules d eau :', report['water_cells'])
    # apercu : eau en bleu, sec en sable (1 pixel = 12 m), reperes des zones du desert
    ...
    for z in range(N):
        for x in range(N):
            w = wet(ORIGIN[0] + x * 12 + 6, ORIGIN[1] + z * 12 + 6)
            px[x, z] = (40, 110, 170) if w else (220, 200, 150)
    d = ImageDraw.Draw(im)
    for name, x, z in (('A', 1372.6, 814.3), ('B', 2860.7, 160.9), ('C', 862.9, 1358.2), ('D', 3821.3, 725.0),
                       ('E', 648.2, 2592.8), ('G', 3151.8, 2513.4), ('H', 3118.8, 3533.7), ('porte', 2266.4, 258.1)):
        cx, cz = (x - ORIGIN[0]) / 12, (z - ORIGIN[1]) / 12
        d.ellipse((cx - 3, cz - 3, cx + 3, cz + 3), fill=(200, 0, 0)); d.text((cx + 4, cz - 6), name, fill=(120, 0, 0))
    im.resize((768, 768)).save(ROOT / 'models-v2/ocean-coast-mask-desert.png')

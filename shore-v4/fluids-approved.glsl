...
vec3 shadeModernWater(vec3 P, vec3 geometricNormal, vec4 legacyTexture) {
  ...
  // Soft contact foam rather than a uniform pale veil.
  float edge = (1.0-smoothstep(0.04,0.28,thickness))*horizontal;
  // Impact foam is handled by the advected patches, not a flat shoreline decal.
  for (int i=0; i<water_impact_count; i++) {
    if (abs(P.y-water_impacts[i].y)<0.35) {
      edge *= smoothstep(0.2,1.1,length(P.xz-water_impacts[i].xz));
    }
  }
  float foam = edge*smoothstep(0.48,0.74,fluidNoise(local*10.0+fluid_time*0.25))*0.32;
  foam += wakeFoam*smoothstep(0.25,0.65,fluidNoise(local*19.0+fluid_time*vec2(1.9,0.7)));

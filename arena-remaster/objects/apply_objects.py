...
def main(parts, textures=None):
    ...
    for level in sorted({level_of(t) for t in hd} - {'wasstada'}):
        ...
        (H / f'{level}-patch.json').write_text(json.dumps({'description': f'Arene : textures HD ({textures})', 'remove': [], 'add': [],
                                                           'textures': [t for t in hd if level_of(t) == level]}))

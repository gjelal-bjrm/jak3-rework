    ...
    if a.lod<3:
        ...
        for k in range(nr):
            ...
            for j in range(ns):
                ...
                def scar_uv(point):
                    # A scar continues the trunk's 3x7 cylindrical grain. Its
                    # four triangles share UVs at every shared position, rather
                    # than mirroring a fresh texture triangle toward the nose.
                    theta=math.atan2(point.z-c.z,point.x-c.x)
                    theta=angle+math.atan2(math.sin(theta-angle),math.cos(theta-angle))
                    return (theta/(2*math.pi)*3,(point.y-y0)/(y1-y0)*7)
                for m in range(4):
                    triangle=[outline[m],outline[(m+1)%4],nose]
                    a.face(triangle,[scar_uv(point) for point in triangle],True)

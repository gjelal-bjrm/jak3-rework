...
def main():
 ...
 parent=H.parent/'architecture/revision-002/installed.json';prec=read(parent)
 ...
 manifest={'status':'staged','path':'out/jak3/fr3/wascitya.fr3','base_path':str(before),'base_sha256':sha(before),
  ...
  'native_visual_validation':False,'scope':'Close house1 east-room window and remove its frame and balcony by replaying native facade author without this opening. Keep west and upper windows, all house2 and other geometry unchanged.'}
 for field,filename in [('closed_window_validation','closed-window-validation.json'),('bvh_coverage','bvh-coverage.json'),('opening_validation','opening-validation.json'),('geometry_validation','geometry-validation.json'),('window_anchors','window-anchors.json')]:

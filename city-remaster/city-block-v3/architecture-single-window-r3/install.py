"""Install the single-window correction over the exact recorded R2 map."""
...
def main():
    ...
    for field in ('base_path','output_path','patch','preservation_report','parent_record',
                  'author_report','closed_window_validation','bvh_coverage',
                  'opening_validation','geometry_validation','window_anchors'):
    ...
    record=H.parent/'architecture/revision-003/installed.json'
    assert not record.exists(),'Revision already installed'
    protected={str(R/'variants'/v/candidate['path']):sha(R/'variants'/v/candidate['path']) for v in ('original','remaster-v1')}
    ...
    print('Installed single-window correction:',sha(live))

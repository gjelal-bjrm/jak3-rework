"""Rebuild six house2 metal plaques and remove superseded motif fragments."""
...
def remodel(component,targets,wall,lod,name):
 ...
 n=Vector((near[1].x,0,near[1].z)).normalized();r=Vector((n.z,0,-n.x));u=Vector((0,1,0))
 ...
 for f in add:
  ...
  for v in f['vertices']:v['normal']=list(nn)
 ...
 return add,{'name':name,'width_m':w,'height_m':height,'center_m':list(center),'normal':list(n),'backing_offset_m':.022,'front_offset_m':.272,'ridge_offset_m':.372,'added':len(add),'closed_mesh':True}
...
def main():
 ...
 for lod in range(4):
  ...
  # Remove the whole old motif crossing the new upper window, plus the
  # separate small fragments from the superseded decorative assembly.
  for d,g in [(draw,10),(41 if lod<3 else 39,4),(41 if lod<3 else 39,5),(41 if lod<3 else 39,6)]:
   fs=[f for f in current['faces'] if f['tree_type']=='tie' and f['geom']==lod and f['tree']==0 and f['draw']==d and f['group']==g]
   assert fs,(lod,d,g)
   patch['remove'].extend({**{k:f[k] for k in ('tree_type','geom','tree','draw','group','stream_index')},'original_positions':[v['p'] for v in f['vertices']]} for f in fs)
   report['deleted_groups'].append({'lod':lod,'draw':d,'group':g,'triangles':len(fs)})

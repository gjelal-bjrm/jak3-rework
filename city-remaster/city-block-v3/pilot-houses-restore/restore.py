"""Rend aux deux maisons pilotes de la ville basse (WCA, TIE arbre 1, instances 1 et 2) leur
...
INSTANCES = (1, 2); TREE = 1
PILOT_PREFIXES = ('wca-house1-', 'wca-house2-')
...
def build_patch():
    current = read(H / 'native-pilot-current.json'); original = read(C / 'environment/architecture/wascitya-native.json')
...
def stage(patch):
    ...
    candidate = {'status': 'staged', 'path': 'out/jak3/fr3/wascitya.fr3', 'base_path': str(before), 'base_sha256': sha(before),
                 ...
                 'parent_record': str(BLOCK / 'architecture/revision-004/installed.json'), 'parent_record_sha256': sha(BLOCK / 'architecture/revision-004/installed.json'),
                 'bvh_coverage': str(H / 'bvh-coverage.json'), 'bvh_coverage_sha256': sha(H / 'bvh-coverage.json'),
                 'scope': 'Maisons pilotes 1 et 2 (WCA, TIE arbre 1) rendues a leur geometrie native ; leurs cinq ouvertures disparaissent.'}
...
def install(candidate):
    ...
    r4 = read(BLOCK / 'architecture/revision-004/installed.json'); market = read(BLOCK / 'market-windows-r5/installed.json')
    kept = [w for w in read(r4['window_anchors']) if not w['id'].startswith(PILOT_PREFIXES)] + read(BLOCK / 'market-windows-r5/window-anchors.json')
    final = BLOCK / 'interiors/window-anchors-final.json'; write(final, kept)

...
def main():
 ...
 parent=H.parent/'architecture/revision-003/installed.json';prec=read(parent)
 ...
 manifest={'status':'staged','path':'out/jak3/fr3/wascitya.fr3','base_path':str(before),'base_sha256':sha(before),
  ...
  'native_visual_validation':False,'scope':'Restore exact R2 house1 lateral opening and add four street-facing openings on four distinct native houses; six houses and nine total apertures. Preserve all unselected native geometry, palettes, collision and BVH.'}
 for field,filename in [('restoration_validation','restoration-validation.json'),('street_visibility_validation','street-visibility-validation.json'),('bvh_coverage','bvh-coverage.json'),('opening_validation','opening-validation.json'),('geometry_validation','geometry-validation.json'),('window_anchors','window-anchors.json')]:

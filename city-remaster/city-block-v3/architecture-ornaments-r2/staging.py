...
def main():
 ...
 parent=H.parent/'architecture/revision-001/installed.json';prec=read(parent)
 ...
 manifest={'status':'staged','path':'out/jak3/fr3/wascitya.fr3','base_path':str(before),'base_sha256':sha(before),
  ...
  'native_visual_validation':False,'scope':'Reanchor four closed house2 metal plaques to street side of unchanged native wall. Only draw40/38 groups5/6/7; no wall/window/material/BVH changes'}
 for field,filename in [('visibility_validation','visibility-validation.json'),('ornament_validation','ornament-validation.json'),('bvh_coverage','bvh-coverage.json'),('opening_validation','opening-validation.json'),('geometry_validation','geometry-validation.json'),('window_anchors','window-anchors.json')]:

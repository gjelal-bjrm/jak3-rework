...
for lod,fs in groups.items():
 ...
 checks={'four_closed_components':len(parts)==4,'every_edge_two_faces':all(n==2 for n in edges.values()),'positive_component_volumes':min(volumes)>0,'finite_vertices':all(math.isfinite(x) for p in ps for x in p)}
 report['checks'].update({str(lod)+'/'+k:v for k,v in checks.items()});report['lods'].append({'lod':lod,'components':len(parts),'vertices':len(ps),'triangles':len(fs),'volumes_m3':volumes,'native_winding_signed_distance_min_m':minimum,'native_winding_signed_distance_max_m':maximum,'note':'Native walls are double sided. Visibility, not native winding, establishes correct street side.'})

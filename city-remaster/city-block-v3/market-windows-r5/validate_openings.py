"""Controle des ouvertures par lancers de rayons sur les triangles source + remplacement (WCB marche).

Reprise de architecture-expanded-r4/validate_openings.py avec les fichiers de ce lot.
...
source = read(H / 'native-market.json'); patch = read(H / 'wascityb-patch.json'); anchors = read(H / 'window-anchors.json')
        ...
        for x in (-.35, 0, .35):
            for y in (-.35, 0, .35):
                ...
                if hit[0] is not None:
                    f = faces[hit[2]]
                    hits.append({'sample': [x, y], 'point': list(hit[0]), 'distance': hit[3], 'material': f.get('material'),
                                 'tree': f['tree'], 'draw': f['draw'], 'group': f['group'], 'key': list(key(f)) if 'stream_index' in f else None})
        checks.append({'name': w['id'] + ' LOD' + str(lod) + ' actual 3x3 rays clear', 'passed': not hits, 'hits': hits})
result = {'passed': all(c['passed'] for c in checks), 'source_sha256': source['source_fr3']['sha256'],
          'patch_sha256': hashlib.sha256((H / 'wascityb-patch.json').read_bytes()).hexdigest(), 'checks': checks}
(H / 'opening-validation.json').write_text(json.dumps(result, indent=2) + '\n')
print(json.dumps({'passed': result['passed'], 'failed': [c['name'] for c in checks if not c['passed']]}, indent=2))

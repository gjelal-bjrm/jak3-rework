...
class CityInteriors {
 public:
  ...
  void render(SharedRenderState* state){
    ...
    if(!m_shader||!m_shader->okay()||m_windows.empty())return;
    ...
    for(size_t i=0;i<m_windows.size();++i){
      const auto& w=m_windows[i];float d2=0,front=0;
      for(int c=0;c<3;++c){float d=eye[c]-w.center[c];d2+=d*d;front+=d*w.basis[6+c];}
      if(d2>110.f*110.f||front<-.4f)continue;
      glUniformMatrix3fv(location("room_basis"),1,GL_FALSE,w.basis.data());glUniform3fv(location("room_origin"),1,w.origin.data());
      glUniform1f(location("room_occupancy"),w.inhabited?1.f:.2f);
      bool sitting=w.depth_scale>.8f&&((i+m_scenario)%2)==0;
      draw(m_meshes.at(sitting?"lounge":"conversation"),{0,0,0},{1,w.height_scale,w.depth_scale},0,seconds);
      if(w.inhabited&&d2<75.f*75.f){
        if(sitting)draw(m_meshes.at("sitting-male"),{0,.09f,-2.5f},{1,1,1},0,seconds+float(i));
        else {
          draw(m_meshes.at("conversing-male"),{-.68f,.058f,-1.8f*w.depth_scale},{1,1,1},1.05f,seconds+float(i));
          draw(m_meshes.at("conversing-female"),{.68f,.058f,-1.8f*w.depth_scale},{1,1,1},-1.05f,seconds+float(i)+1.3f);
        }
      }
      ++visible;
    }
 ...
 private:
  ...
  void initialize(SharedRenderState* s){
    ...
    for(const auto& a:config.at("windows")){Window w;w.center=a.at("center").get<Vec3>();auto n=a.at("normal").get<Vec3>(),r=a.at("right").get<Vec3>();
      ...
      float h=a.at("height");w.origin[1]-=h*.5f+.1f;w.height_scale=(h+.6f)/3.65f;w.depth_scale=std::min(1.f,a.value("room_depth",3.65f)/3.65f);w.inhabited=a.at("inhabited");m_windows.push_back(w);
    }
    m_scenario=int(std::chrono::steady_clock::now().time_since_epoch().count()&1);m_start=std::chrono::steady_clock::now();
  }
  ...
  bool m_attempted=false,m_logged=false;int m_scenario=0;GLuint m_vao=0,m_program=0;
  std::unique_ptr<Shader>m_shader;std::map<std::string,Mesh>m_meshes;std::map<std::string,GLuint>m_textures;std::vector<Window>m_windows;

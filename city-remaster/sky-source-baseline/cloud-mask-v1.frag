...
void main() {
  ...
  for(int sampleIndex=0;sampleIndex<10;sampleIndex++) {
    ...
    vec3 p=vec3(uv,z*.17);
    float volume=.56*cloudNoise3(p,13.0,vec3(.004,-.003,.004)*cloud_time+11.1)
                +.29*cloudNoise3(p,29.0,vec3(-.007,.005,-.003)*cloud_time+27.4)
                +.15*cloudNoise3(p,61.0,vec3(.012,.007,.006)*cloud_time+41.3);
    float density=max(0.0,1.55*broad-.43+.62*(billow-.5)+1.35*(volume-.5)-.74*z*z)*envelope;
    float stepOpacity=1.0-exp(-density*.52);
    // Broad ambient scattering only; directional sun/moon light still comes
    // from the two native GOAL layers and their time-of-day vertex colours.
    float ambient=mix(.50,1.28,smoothstep(-.65,.85,z));
  ...
  opacity=max(opacity,.26*original*envelope);
  bodyShade=mix(1.0,clamp(bodyShade,.65,1.18),smoothstep(.001,.04,opacity));

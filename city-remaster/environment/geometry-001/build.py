...
def stage(level):
    ...
    parts=[ENV/'architecture'/(level+'-patch.json'),ENV/'architecture/windows'/(level+'-patch.json'),ENV/'vegetation-v2'/(level+'-patch.json')]
    assert all(p.exists() for p in parts),'Wait for all independently authored parts'
    facade=read(ENV/'architecture'/(level+'-independent-validation.json'))
...
def deploy(apply):
    ...
    for r in records:
        ...
        for tree in ('data','variants/remaster'):assert sha(ROOT/tree/r['path'])==r['base_sha256']
    ...
    try:
        ...
        for r in records:
            for tree in ('data','variants/remaster'):copy(r['base_path'],ROOT/tree/r['path'])
